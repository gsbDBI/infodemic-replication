...
test.object <- t.test(x)
test.object
(estimate <- mean(x))
(std.error <- sd(x)/sqrt(length(x)))
# pvalue greater than
1-pt(estimate/std.error, df = length(x) - 1, lower = FALSE)
# pvalue less than
pt(estimate/std.error, df = length(x) - 1, lower = FALSE)
# pvalue two-sided
2*(pt(abs(estimate/std.error), df = length(x) - 1, lower = FALSE))
