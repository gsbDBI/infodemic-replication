...
gg <- ggplot(df_grid, aes(x = estimate, y = term, group = colors, color = colors,
  ...
  geom_text(aes(x = position_x, y = term, label = round(Probabilities,3)),
            position=position_dodge(width=1.1), size = 2.75,
            hjust = 0) + 
...
gt$heights[13] = 0.35*gt$heights[13] 
gt$heights[18] = 0.35*gt$heights[18] 
...
# Figure 4: Selected covariate means by restricted optimal policy assignment
...
# Table S8: Alternative specifications
...
# Table S9: Control response and treatment effect estimates, secondary targeted policy
...
if(file.exists('objects/aipw_scores_post_true_restricted.RDS')){ # read in scores if already generated
  aipw_scores_post_true_restricted <- readRDS('objects/aipw_scores_post_true_restricted.RDS')
  
}else{
  aipw_scores_post_true_restricted <- aw_scores_eval(xs = xs_eval,
                                                     yobs = df_eval$post_true, 
                                                     ws = as.factor(case_when(ws_eval == 1 ~ 1,
                                                                              ws_eval == optimal_assignment ~ 2,
                                                                              TRUE ~ 3)), 
  ...
  saveRDS(aipw_scores_post_true_restricted, 'objects/aipw_scores_post_true_restricted.RDS')
  
}

if(file.exists('objects/aipw_scores_post_false_restricted.RDS')){ # read in scores if already generated
  aipw_scores_post_false_restricted <- readRDS('objects/aipw_scores_post_false_restricted.RDS')
  
}else{
  aipw_scores_post_false_restricted <- aw_scores_eval(xs = xs_eval,
                                                      yobs = df_eval$post_false, 
                                                      ws = as.factor(case_when(ws_eval == 1 ~ 1,
                                                                               ws_eval == optimal_assignment ~ 2,
                                                                               TRUE ~ 3)), 
  ...
  saveRDS(aipw_scores_post_false_restricted, 'objects/aipw_scores_post_false_restricted.RDS')
...
```{r post_truefalse_plots, cache = TRUE, eval = FALSE}

# save estimates
est_list <- list(
  cbind(aipw_scores_post_true[, 2:5], aipw_scores_post_true_learned[, 2], aipw_scores_post_true_restricted[, 2]),
  cbind(aipw_scores_post_false[, 2:5], aipw_scores_post_false_learned[, 2], aipw_scores_post_false_restricted[, 2])
)
...
# Table S10: Heterogeneity, averaged respondent
...
# Table S11: Heterogeneity, accuracy
...
# Table S12: Heterogeneity, Facebook
...
pt(mean(x)/(sd(x)/sqrt(length(x))), df = length(x)-1, lower = FALSE)
...
# The accuracy nudge is relatively more effective at moving false sharing intentions on timeline
...
# Differences in effects between channels within treatment conditions are not stat stig
