...
df_treat$hhi <- df_treat$hhi_AB
df_treat$pol <- df_treat$pol_AB
...
df_treat$country <- ifelse(df_treat$nigeria ==1 , "Nigeria", "Kenya")
ab$country <- ab$COUNTRY

# subset experimental + AB data to variables of interest
df_treat_dem <- df_treat[,c(compare_cols, 'batch')]
...
new_gg <- aggregate(y_on ~ batch, data = gg_df, function(x) c(estimate = mean(x), std.error = sd(x)/sqrt(length(x))))
new_gg <- do.call(data.frame, new_gg)
new_gg <- setNames(new_gg, c('Batch', 'estimate', 'std.error'))

ggplot(new_gg, aes(x = Batch, y = estimate)) + 
  geom_point() + 
  geom_line() + 
  stat_gradientinterval(aes(x = Batch, 
                            ydist = distributional::dist_normal(estimate, std.error)), .width = 0, size = 0) +
  geom_errorbar(aes(ymin = estimate - 1.96*std.error, 
                    ymax = estimate + 1.96*std.error), width = 0.05) +
  ylab('Average overlap') + 
  xlab('Batch') +
  geom_hline(yintercept = 1/8, colour = 'grey60', linetype = 2) + 
  theme_minimal() + 
  theme(plot.title = element_text(hjust = 0.5),
        panel.background = element_rect(fill = 'white'),
        plot.background = element_rect(fill = 'white', color = 'white'),
...
ggsave('../figures/on_policy.png', width = 8, height = 4)
