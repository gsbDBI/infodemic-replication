...
gg <- ggplot(df_grid, aes(x = estimate, y = term, group = colors, color = colors,
  ...
  xlab('Estimate') +
  ylab('Policy') +
  scale_color_manual(values = cbPalette[-1], breaks = estimate_order[c(1, 9:12)]) +
  scale_fill_manual(values = cbPalette[-1], breaks = estimate_order[c(1, 9:12)]) +
...
# gtable::gtable_show_layout(gt) # to check which heights to change
...
rows <- as.data.frame(matrix(c('', '', '', '\\textbf{False}', '', '', '\\textbf{True}', '',
...
all_scores <- bind_rows(list(
  `Any` = bind_rows(list(`Combined` = aipw_est), 
                    .id = 'Response measure'), 
...
ggplot(all_scores, aes(x = estimate, y =term, color=`Response measure`, 
  ...
  xlab('Estimate') +
  ylab('Policy') +
  scale_color_manual(values = cbPalette[c(7,6,2)], breaks = c('False', 'True', 'Combined')) +
  scale_fill_manual(values = cbPalette[c(7,6,2)], breaks = c('False', 'True', 'Combined')) +
  theme_minimal() + 
  theme(plot.title = element_text(hjust = 0.5),
        panel.background = element_rect(fill = 'white'),
        plot.background = element_rect(fill = 'white', color = 'white'),
...
xx <- modelsummary(models, 
                   ...
                   coef_rename = c('Headline\nFactcheck' = r'(\hspace{1em}Factcheck)',
                                   ...
                                   'Respondent\nAlternative targeted' = r'(\hspace{1em}Alternative targeted policy)',
...
xx <- gsub(xx_substr,
           paste0(xx_substr, ' \\hspace{1.5em}(maximizing combined response)'),
           xx, fixed = TRUE)

yy <- gregexec('Alternative targeted policy[& -\\{num}{0-9.*}]*', xx)
...
# Figure 4: Selected covariate means by alternative optimal policy assignment
...
for(i in 1:length(WR_idx)){
  x <- WR_idx[[i]]
  probs_r[,i] <- rowSums(probs_learn[,x])
}
gg_df <- tibble(batch = df_learn$batch,
                y_on = ifelse(predict(cf.priority, xs_learn)$predictions<0, probs_r[,2], probs_r[,7]))

new_gg <- list(x = lm_robust(y_on ~ as.factor(batch) -1, data = gg_df))

rows <- data.frame(matrix(c('', 'row1'), ncol = 2) )
...
xx <- gsub('\\toprule', ' & \\textbf{On policy} \\\\ \n \\cmidrule(lr){2-2}', xx[1], fixed = TRUE)
...
# Table S7: Alternative specifications
...
# alternative optimal policy
if(file.exists('objects/aipw_scores_alternative_unbalanced.RDS')){ # read in scores if already generated
  aipw_scores_alternative_unbalanced <- readRDS('objects/aipw_scores_alternative_unbalanced.RDS')
  
}else{
  aipw_scores_alternative_unbalanced <- aw_scores_eval(xs = xs_eval,
                                                       yobs = yobs_eval, 
                                                       ws = as.factor(case_when(ws_eval == 1 ~ 1,
                                                                                ws_eval == optimal_assignment ~ 2,
                                                                                TRUE ~ 3)))
  
  saveRDS(aipw_scores_alternative_unbalanced, 'objects/aipw_scores_alternative_unbalanced.RDS')
...
if(file.exists('objects/aipw_scores_imputed.RDS')){ # read in scores if already generated
  ...
  aipw_scores_imputed <- aw_scores_eval(
    ...
    ws = as.factor(case_when(as.numeric(df_treat[which(df_treat$batch == 5),'W']) == 1 ~ 1, # control
                             as.numeric(df_treat[which(df_treat$batch == 5),'W']) == 2 ~  2, # headline factcheck,
...
# alternative optimal policy
if(file.exists('objects/aipw_scores_alternative_imputed.RDS')){ # read in scores if already generated
  aipw_scores_alternative_imputed <- readRDS('objects/aipw_scores_alternative_imputed.RDS')
  
}else{
  aipw_scores_alternative_imputed <- aw_scores_eval(
    xs = as.matrix(df_treat[which(df_treat$batch == 5), c(context_cols, predv_cols)]),
    yobs = df_treat[which(df_treat$batch == 5),'Y'], 
    ws = 
      as.factor(case_when(as.numeric(df_treat[which(df_treat$batch == 5),'W']) == 1 ~ 1,
  ...
  saveRDS(aipw_scores_alternative_imputed, 'objects/aipw_scores_alternative_imputed.RDS')
...
# alternative optimal policy
if(file.exists('objects/aipw_scores_alternative_adjusted.RDS')){ # read in scores if already generated
  aipw_scores_alternative_adjusted <- readRDS('objects/aipw_scores_alternative_adjusted.RDS')
  
}else{
  aipw_scores_alternative_adjusted <- aw_scores_eval(xs = xs_eval,
                                                     yobs = yobs_eval, 
                                                     ws = as.factor(case_when(ws_eval == 1 ~ 1,
                                                                              ws_eval == optimal_assignment ~ 2,
                                                                              TRUE ~ 3)), 
  ...
  saveRDS(aipw_scores_alternative_adjusted, 'objects/aipw_scores_alternative_adjusted.RDS')
...
aipw_te_alternative_unbalanced <- find_te(aipw_scores_alternative_unbalanced)
...
ws_alternative <- as.factor(case_when(ws_eval == 1 ~ 1,
                                      ws_eval == optimal_assignment ~ 2,
                                      TRUE ~ 3))
...
l0l <- lm_robust(Y~ws_original, data = df_eval,
                 weights = balwts_eval)
l0a <- lm_robust(Y~ws_alternative, data = df_eval,
                 weights = balwts_eval)
...
l1a <- lm_lin(Y~ws_alternative, data = df_eval,
              weights = balwts_eval, 
              covariates = formula(' ~ pre_false + pre_true'))
...
l2a <- lm_lin(Y~ws_alternative, data = df_eval,
              weights = balwts_eval, 
              covariates = formula(' ~ as.factor(optimal0)'))
...
row2 <- '& \\multicolumn{3}{c}{Scores} &  \\multicolumn{1}{c}{Robust OLS} & \\multicolumn{2}{c}{Lin estimator}\\\\
\\cmidrule(lr){2-4} \\cmidrule(lr){5-5} \\cmidrule(lr){6-7} \\multicolumn{4}{l}{\\textbf{Headline treatment effects}} \\rule{0pt}{1.2\\normalbaselineskip}\\\\'
...
\\hspace{1em} All & Yes & Yes & Yes & No & No & No \\\\\n
\\hspace{1em} Pre-test response only & No & No & No & No & Yes & No \\\\\n
\\hspace{1em} Learned policy strata only & No & No & No & No & No & Yes \\\\\n
{\\textbf{Missing data}} \\rule{0pt}{1.2\\normalbaselineskip}\\\\\n
\\hspace{1em} Imputing pre-test response & No & Yes & 
No & No & No & No \\\\\n
{\\textbf{Weights}} \\rule{0pt}{1.2\\normalbaselineskip}\\\\\n
\\hspace{1em} Treatment probability  & Yes & Yes & Yes & Yes & Yes & Yes \\\\\n
\\hspace{1em} Learning stage covariate distribution & No & No & Yes & No & No & No \\\\\n
\\hspace{1em} Censoring & No & No & No & No & No & No 
\\\\\n
{\\textbf{N}} & 10,531 & 11,534  & 10,531 & 10,531 & 10,531 & 10,531 
\\rule{0pt}{1.2\\normalbaselineskip}\\\\\n', xx[1], fixed = TRUE)
...
# Table S8: Control response and treatment effect estimates, secondary targeted policy
```{r learning_policy_combined_alternative, cache = TRUE}
...
# Compute a prioritization based on estimated treatment effects.
priority.cate_c_alt <- predict(cf.priority_c_alt, xs_eval[test_idx,])$predictions

# Estimate AUTOC on held out data.
cf.eval_c_alt <- causal_forest(
  X = xs_eval[test_idx,],
  Y = df_eval$Y[test_idx],
  W = 1*(ws_eval[test_idx]==4),
  W.hat = rowSums(treat_uncens_prob[test_idx, c(4), drop = FALSE])/rowSums(treat_uncens_prob[test_idx, c(4,5)]),
  #W.hat = rowSums(probs_eval[test_idx, c(6), drop = FALSE])/rowSums(probs_eval[test_idx, c(6, 11)]),
  #sample.weights = balwts_uncens[test_idx],
)


rate_c_alt <- rank_average_treatment_effect(cf.eval_c_alt, priority.cate_c_alt, 
                                            q = seq(0.01, 1, by = 0.01), 
                                            R = 800)
rate_c_alt
plot(rate_c_alt)

rate_c_alt$estimate/rate_c_alt$std.err


```

```{r scores_policy_combined_alternative, cache = TRUE}
# alternative optimal policy, combined response function
...
```{r table_policy_combined_alternative, cache = TRUE}
# Mean response estimates
models <- list(
  `Combined` = list(
...
# Figure S5: Significance of treatment effects under alternative weighting schemes

```{r post_truefalse_scores, cache = TRUE}


if(file.exists('objects/aipw_scores_post_true.RDS')){ # read in scores if already generated
  aipw_scores_post_true <- readRDS('objects/aipw_scores_post_true.RDS')
  
}else{
  aipw_scores_post_true <- aw_scores_eval(xs = xs_eval,
                                          yobs = df_eval$post_true, 
                                          ws = ws_eval, 
                                          sample.weights = balwts_uncens)
  
  saveRDS(aipw_scores_post_true, 'objects/aipw_scores_post_true.RDS')
  
}

if(file.exists('objects/aipw_scores_post_false.RDS')){ # read in scores if already generated
  aipw_scores_post_false <- readRDS('objects/aipw_scores_post_false.RDS')
  
}else{
  aipw_scores_post_false <- aw_scores_eval(xs = xs_eval,
                                           yobs = df_eval$post_false, 
                                           ws = ws_eval, 
                                           sample.weights = balwts_uncens)
  
  saveRDS(aipw_scores_post_false, 'objects/aipw_scores_post_false.RDS')
  
}

if(file.exists('objects/aipw_scores_post_true_alternative.RDS')){ # read in scores if already generated
  aipw_scores_post_true_alternative <- readRDS('objects/aipw_scores_post_true_alternative.RDS')
  
}else{
  aipw_scores_post_true_alternative <- aw_scores_eval(xs = xs_eval,
                                                      yobs = df_eval$post_true, 
                                                      ws = as.factor(case_when(ws_eval == 1 ~ 1,
                                                                               ws_eval == optimal_assignment ~ 2,
                                                                               TRUE ~ 3)), 
  ...
  saveRDS(aipw_scores_post_true_alternative, 'objects/aipw_scores_post_true_alternative.RDS')
  
}

if(file.exists('objects/aipw_scores_post_false_alternative.RDS')){ # read in scores if already generated
  aipw_scores_post_false_alternative <- readRDS('objects/aipw_scores_post_false_alternative.RDS')
  
}else{
  aipw_scores_post_false_alternative <- aw_scores_eval(xs = xs_eval,
                                                       yobs = df_eval$post_false, 
                                                       ws = as.factor(case_when(ws_eval == 1 ~ 1,
                                                                                ws_eval == optimal_assignment ~ 2,
                                                                                TRUE ~ 3)), 
  ...
  saveRDS(aipw_scores_post_false_alternative, 'objects/aipw_scores_post_false_alternative.RDS')
  
}


if(file.exists('objects/aipw_scores_post_true_learned.RDS')){ # read in scores if already generated
  aipw_scores_post_true_learned <- readRDS('objects/aipw_scores_post_true_learned.RDS')
  
}else{
  aipw_scores_post_true_learned <- aw_scores_eval(xs = xs_eval,
                                                  yobs = df_eval$post_true, 
                                                  ws = as.factor(case_when(ws_eval == 1 ~ 1,
                                                                           ws_eval == optimal_assignment_original ~ 2,
                                                                           TRUE ~ 3)), 
  ...
  saveRDS(aipw_scores_post_true_learned, 'objects/aipw_scores_post_true_learned.RDS')
  
}

if(file.exists('objects/aipw_scores_post_false_learned.RDS')){ # read in scores if already generated
  aipw_scores_post_false_learned <- readRDS('objects/aipw_scores_post_false_learned.RDS')
  
}else{
  aipw_scores_post_false_learned <- aw_scores_eval(xs = xs_eval,
                                                   yobs = df_eval$post_false, 
                                                   ws = as.factor(case_when(ws_eval == 1 ~ 1,
                                                                            ws_eval == optimal_assignment_original ~ 2,
                                                                            TRUE ~ 3)), 
  ...
  saveRDS(aipw_scores_post_false_learned, 'objects/aipw_scores_post_false_learned.RDS')
...
```{r post_truefalse_plots, cache = TRUE}

# save estimates
est_list <- list(
  cbind(aipw_scores_post_true[, 2:5], aipw_scores_post_true_learned[, 2], aipw_scores_post_true_alternative[, 2]),
  cbind(aipw_scores_post_false[, 2:5], aipw_scores_post_false_learned[, 2], aipw_scores_post_false_alternative[, 2])
)



false_weights <- seq(-1, -0.1, .01)
true_weights <- seq(0.1, 1, .01)

xy <- expand.grid(false_weights, true_weights)


out_l <- mapply(function(x, y) {
  yobs_z <- x*df_eval$post_true + y*df_eval$post_true
...
# out_l <- lapply(1:6, function(s){
#   mapply(function(x, y) {
#     z <- x * est_list[[2]][,s] + y * est_list[[1]][,s]
#     c(estimate = mean(z), 
#       std.error = sd(z)/sqrt(length(z)),
#       false_w = x, 
#       true_w = y,
...
# Table S9: Heterogeneity, averaged respondent
...
# Table S10: Heterogeneity, accuracy
...
# Table S11: Heterogeneity, Facebook
...
### Overlap
```{r overlap_policy}
# Overlap between learned targeted policy and alternative targeted policy
...
round(mean(x)*100,1)
round(sd(x)/sqrt(length(x))*100,1)
...
```{r prop_treatment}
# proportion of respondents in each condition in alternative policy
round(prop.table(table(optimal_assignment)),3)*100
```
...
# fb
x <- (aipw_scores_any_false_alternative[,2] - aipw_scores_any_false[,5])
round(mean(x)*100,1)
round(sd(x)/sqrt(length(x))*100,1)

pt(mean(x)/(sd(x)/sqrt(length(x))), df = length(x)-1, lower = TRUE)

# accuracy
x <- (aipw_scores_any_false_alternative[,2] - aipw_scores_any_false[,4])
round(mean(x)*100,1)
round(sd(x)/sqrt(length(x))*100,1)
...
# 
# The alternative targeted policy is also more effective at moving the combined 
# response function than the original learned targeted policy 

x <- (aipw_scores_alternative[,2] - aipw_scores_learned[,2])
round(mean(x)*100,1)
round(sd(x)/sqrt(length(x))*100,1)
...
# accuracy
x <- (aipw_scores_alternative[,2] - aipw_scores[,4])
round(mean(x)*100,1)
round(sd(x)/sqrt(length(x))*100,1)

1-pt(mean(x)/(sd(x)/sqrt(length(x))), df = length(x)-1, lower = TRUE)
```
...
# CHANNEL SHARING
# the accuracy nudge has larger effects on mitigating false sharing intentions relative to Facebook tips
x <- (aipw_scores_any_false[,4] - aipw_scores_any_false[,5])
round(mean(x)*100,1)
round(sd(x)/sqrt(length(x))*100,1)

# and also true
x <- (aipw_scores_any_true[,4] - aipw_scores_any_true[,5])
round(mean(x)*100,1)
round(sd(x)/sqrt(length(x))*100,1)

# combined
x <- (aipw_scores[,4] - aipw_scores[,5])
round(mean(x)*100,1)
round(sd(x)/sqrt(length(x))*100,1)
...
# The accuracy nudge is relatively more effective at moving false sharing intentions on time 
x <- (aipw_scores_timeline_false[,4] - aipw_scores_timeline_false[,5])
round(mean(x)*100,1)
round(sd(x)/sqrt(length(x))*100,1)
...
x <- (aipw_scores_send_false[,5] - aipw_scores_send_false[,4])
round(mean(x)*100,1)
round(sd(x)/sqrt(length(x))*100,1)
```
...
l1[l1$term == 'Control','std.error']/l0[l0$term == 'Control','std.error']
...
covariate <- 'dli'
adjustment <--1e-5*c(2*(median(df_eval[,covariate])==quantile(df_eval[,covariate], 1))-1)
x <- aipw_scores_any_false[,5]-aipw_scores_any_false[,4]
...
covariate <- 'dli'
adjustment <--1e-5*c(2*(median(df_eval[,covariate])==quantile(df_eval[,covariate], 1))-1)
x <- aipw_scores_timeline_false[,5]-aipw_scores_send_false[,5]
...
covariate <- 'dli'
adjustment <--1e-5*c(2*(median(df_eval[,covariate])==quantile(df_eval[,covariate], 1))-1)
x <- aipw_scores_timeline_false[,4]-aipw_scores_send_false[,4]
