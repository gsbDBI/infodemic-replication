...
models <- list(
  `Combined` = list(tidy = bind_rows(aipw_eval_te,  aipw_eval_est[1,])),
  `False: Any sharing` = list(tidy = bind_rows(aipw_eval_f_te,  aipw_eval_f_est[1,])),
  `False: Messenger` = list(tidy = bind_rows(aipw_eval_sf_te,  aipw_eval_sf_est[1,])),
  `False: Timeline` = list(tidy = bind_rows(aipw_eval_tf_te,  aipw_eval_tf_est[1,])),
  `True: Any sharing` = list(tidy = bind_rows(aipw_eval_t_te,  aipw_eval_t_est[1,])),
  `True: Messenger` = list(tidy = bind_rows(aipw_eval_st_te,  aipw_eval_st_est[1,])),
  `True: Timeline` = list(tidy = bind_rows(aipw_eval_tt_te,  aipw_eval_tt_est[1,]))
)
...
table <- apply(apply_mat, 1, function(x) {
  ...
  if(length(unique(df_eval[, covariate])) <= 2){
    ...
    fmla0 <- formula(paste0('aipw_scores_', channel, '_', stimuli_type, '_eval[,1]', 
                            ...
                            ' ~  factor(',covariate, ') -1'))
    fmla1 <- formula(paste0('aipw_scores_any_',stimuli_type, '_eval[,1]', 
                            ' ~  factor(',covariate, ')'))
    ns <- table(df_eval[,covariate])
  }else{
    # continuous covariates
...
#############
modelsummary(models, stars = TRUE, title = 'Heterogeneity under control, evaluation') %>% 
  row_spec(0, extra_css = "border-bottom: 3px solid")%>% 
...
modelsummary(models, stars = TRUE, title = 'Heterogeneity under control, evaluation', add_rows = row)
...
modelsummary(models, stars = TRUE, title = 'Heterogeneity under control, evaluation', add_rows = row, escape = FALSE) # note: escape = F
...
rows <- tribble(~term, ~Combined, ~a, ~b, ~c, ~d, ~e, ~f,
                ...
                r"(\cmidrule(lr){2-8})", '', '', '', '', '', '', '',
                '\\', '', '', '', '', '', '', '',
                r'(\textbf{Political allegiance})', '', '', '', '', '', '', '',
                r"(\cmidrule(lr){2-8})", '', '', '', '', '', '', '',
                '\\', '', '', '', '', '', '', '',
                r'(\textbf{Digital literacy index})', '', '', '', '', '', '', '',
                r"(\cmidrule(lr){2-8})", '', '', '', '', '', '', '',
                '\\', '', '', '', '', '', '', '',
                r'(\textbf{Scientific knowledge index})', '', '', '', '', '', '', ''
               )

attr(rows, 'position') <- c(1,8,9,
                            10,17,18,
                            19,26,27,
                            28,35,36,
                            37)
