...
df_grid$Headline <- factor(c(gsub(' ', '\n',  WCH_terms), 
                             rep(gsub(' ', '\n',  WCH_terms), each = 7),
                             rep('Average', 8),
                             gsub(' ', '\n',  WCH_terms)), 
                           levels = c(gsub(' ', '\n',  WCH_terms), 'Average'))
df_grid$Respondent <- factor(c(rep(WCR_terms[1],5),
                               rep(gsub(' ', '\n',  WCR_terms)[-1], times = 5),
                               gsub(' ', '\n',  WCR_terms),
                               rep('Average', 5)),
                             levels = c('Average', rev(gsub(' ', '\n',  WCR_terms))))

ggplot(df_grid, aes(x = Headline, y = Respondent, fill = Estimate)) +
  geom_tile(color = "white",
            lwd = 1.5,
            linetype = 1) +
  # coord_fixed() +
  scale_fill_gradientn(colors = 
                         c(cbPalette[7], cbPalette[7], cbPalette[7], cbPalette[2], cbPalette[5], cbPalette[4], cbPalette[4], cbPalette[4])) +
  # scale_fill_continuous(low = cbPalette[7], high = cbPalette[6]) + 
  scale_x_discrete(position = "top", labels = unicode_minus) +
  geom_tile(data = df_grid[which(df_grid$Headline == 'Average' | df_grid$Respondent == 'Average' ),], 
            aes(x = Headline, y = Respondent, fill = Estimate),
            color = cbPalette[1],
            lwd = 1.5,
            linetype = 1) +
  geom_text(aes(label = paste0(unicode_minus(round(Estimate, 3)), '\n',
                               paste0('(', round(std.error, 3), ')')
  )), 
...
ggsave('../figures/joint_scores.png', height = 6, width = 8)

```

# Figure S4: Cumulative assignment
...
ggplot(data = xx[which(xx$`Total observations`<A),], aes(y = `Cumulative assignment counts`, 
  ...
  theme(panel.background = element_rect(fill = 'white'),
        plot.background = element_rect(fill = 'white', color = 'white'))
ggsave('../figures/respondent_base_assignment_on_time.png', width = 8, height = 6)
...
ggplot(data = xx[which(xx$`Total observations`<A),], aes(y = `Cumulative assignment counts`, 
  ...
  theme(panel.background = element_rect(fill = 'white'),
        plot.background = element_rect(fill = 'white', color = 'white'))
ggsave('../figures/headline_base_assignment_on_time.png', width = 8, height = 6)
...
xx <- modelsummary(models, 
                   ...
                   escape = FALSE,
                   col.names = NULL,
                   coef_rename = c('Age: Below median (n = 5,412)' = r'(\hspace{1em} Below median)',
                                   'Age: Above median (n = 5,271)' = r'(\hspace{1em} Above median)',
                                   'Age: Difference' = r'(\hspace{1em} Difference)',
                                   'Male:  Not male (n = 5,050)' = r'(\hspace{1em} Not male)',
                                   'Male: Male (n = 5,633)' = r'(\hspace{1em} Male)',
                                   'Male: Difference' = r'(\hspace{1em} Difference )',
                                   'Supports governing party:  Not aligned (n = 7,570)' = r'(\hspace{1em} Not aligned)',
                                   'Supports governing party: Aligned (n = 3,113)' = r'(\hspace{1em} Aligned)',
                                   'Supports governing party: Difference' = r'(\hspace{1em} Difference  )',
                                   'Digital literacy index: Below median (n = 5,443)' = r'(\hspace{1em} Below median )',
                                   'Digital literacy index: Above median (n = 5,240)' = r'(\hspace{1em} Above median )',
                                   'Digital literacy index: Difference' = r'(\hspace{1em}  Difference)',
                                   'Scientific knowledge index: Below median (n = 5,677)' = r'(\hspace{1em} Below median  )',
                                   'Scientific knowledge index: Above median (n = 5,006)' = r'(\hspace{1em} Above median  )',
...
# Figure 4: covariates by optimal policy
...
own_fun <- function(x) {ifelse(x %in% c("Yes, someone else owns","Yes, do own"),1,0)}
...
ab$hhi <- rowSums(as.data.frame(lapply(ab[hhi_cols], own_fun)))
...
# Figure 1: On-policy in learning
...
# Table S6: Alternative specifications
```{r additional_scores}
...
aipw_te_unbalanced$term <- aipw_te_adjusted$term <-
  treatment_levels[-1]

aipw_est_unbalanced$term <- aipw_est_adjusted$term <-
  treatment_levels[1]
...
rows <- as.data.frame(matrix(c('', '(1)', '(2)', '(3)', '(4)', '(5)',
                               'row2', rep('', 5),
                               'row7', rep('', 5),
                               'row16', rep('', 5)),
                             byrow = TRUE, ncol = 6
...
row2 <- '& \\multicolumn{2}{c}{Scores} &  \\multicolumn{1}{c}{Robust OLS} & \\multicolumn{2}{c}{Lin estimator}\\\\
\\cmidrule(lr){2-3} \\cmidrule(lr){4-4} \\cmidrule(lr){5-6} \\multicolumn{4}{l}{\\textbf{Headline treatment effects}} \\rule{0pt}{1.2\\normalbaselineskip}\\\\'
...
# Alternative policy learning
```{r learning_policy_combined_alternative}
...
```{r scores_policy_combined_alternative}
...
```{r table_policy_combined_alternative}
# Mean response estimates
models <- list(
  `Combined` = list(
...
# Different combined response

```{r post_truefalse_scores}
...
false_weights <- seq(-1, 0, .1)
true_weights <- seq(0, 1, .1)

xy <- expand.grid(false_weights, true_weights)

out_l <- lapply(1:6, function(s){
mapply(function(x, y) {
  # z <- x * aipw_scores_post_false_alternative[,2] + y * aipw_scores_post_true_alternative[,2]
...
# Figure XXX: covariates by optimal policy
```{r covariates_in_optimal}
...
# Table S6: Heterogeneity, averaged respondent
...
# Table S7: Heterogeneity, accuracy
...
# Table S8: Heterogeneity, Facebook
...
# and also true
x <- (aipw_scores_any_true[,5] - aipw_scores_any_true[,4])
mean(x)
sd(x)/sqrt(length(x))
